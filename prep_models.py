...
def prep_cobrapy_models(models,upper_bound_functions = {},lower_bound_functions = {},upper_bound_functions_dt = {},lower_bound_functions_dt = {},extracell = 'e', random_kappas = "new",media = {}, met_filter = [],met_filter_sense = "exclude", lb_funs = "constant", ub_funs = "linearRand"):
    ...
    for modelkey in models.keys():
        ...
        if EyE.values.sum < 0:#(-EyE.values == np.eye(EyE.values.shape[0])).all():
        ...
        for rxn in util.solver.linear_reaction_coefficients(model).keys():
            growth_col.loc[rxn] = rxn in util.solver.linear_reaction_coefficients(model)[rxn]

        lilgamma = growth_col.values

        internal_upper_bounds = np.array([max(0,rxn.bounds[1]) for rxn in model.reactions if rxn.id not in exrn[model.name]])
        internal_lower_bounds = np.array([max(0,-rxn.bounds[0]) for rxn in model.reactions if rxn.id not in exrn[model.name]])
...
######## Upper Bounds
        ...
        if modelkey in upper_bound_functions.keys():
            if isinstance(upper_bound_functions[modelkey],dict):
                ...
                exub = np.array([lambda x : master_y0[metab] for matab in masterlist])
                exubdt = np.array([lambda x : 0  i in range(len(masterlist))])
                ...
                exub = np.array([lambda x : rands[i]*x[i] for i in range(len(masterlist))])
                exubdt = np.array([lambda x,xd : rands[i]*xd[i] for i in range(len(masterlist))])
            elif upper_bound_functions[modelkey] == "linear1":
                print(modelkey, " Upper Bounds: Using linear uptake with uniform coefficients = 1")
                exub = np.array([lambda x : x[i] for  i in range(len(masterlist))])
                exubdt = np.array([lambda x,xd : xd[i] for  i in range(len(masterlist))])
                ...
                rands = [np.random.rand() for i in range(len(masterlist))]
                exub = np.array([lambda x : x[i]/(rands[i] + x[i]) for i in range(len(masterlist))])
                exubdt = np.array([lambda x,xd : xd[i]*rands[i]/((rands[i]+x[i])**2) for i in range(len(masterlist))])
                ...
                exub = np.array([lambda x : x/(1 + x) for i in range(len(masterlist))])
                exubdt = np.array([lambda x,xd : xd[i]*1/((1+x[i])**2) for i in range(len(masterlist))])
            else:
...
######### Lower Bounds
        ...
        if modelkey in lower_bound_functions.keys():
            if isinstance(lower_bound_functions[modelkey],dict):
                ...
                if modelkey in upper_bound_functions_dt.keys():
                    exlbdt = np.array([upper_bound_functions_dt[modelkey][met] if met in upper_bound_functions_dt[modelkey].keys() else lambda x : 0 for met in masterlist])
                else:
                    print("Error: Numeric differentiation not supported. Please provide derivative for user-defined bound functions")
                    return None
            elif lower_bound_functions[modelkey] == "constant":
                print(model.name, " Lower Bounds: Using constant uptake")
                exlb = np.array([lambda x : master_y0[metab] for matab in masterlist])
                exlbdt = np.array([lambda x : 0  i in range(len(masterlist))])
            elif lower_bound_functions[modelkey] == "linearRand":
                print(model.name, " Lower Bounds: Using linear uptake with random coefficients")
                rands = [np.random.rand() for i in range(len(masterlist))]
                exlb = np.array([lambda x : rands[i]*x[i] for i in range(len(masterlist))])
                exlbdt = np.array([lambda x,xd : rands[i]*xd[i] for i in range(len(masterlist))])
            elif lower_bound_functions[modelkey] == "linear1":
                print(model.name, " Lower Bounds: Using linear uptake with uniform coefficients = 1")
                exlb = np.array([lambda x : x[i] for  i in range(len(masterlist))])
                exlbdt = np.array([lambda x,xd : xd[i] for  i in range(len(masterlist))])
                ...
                rands = [np.random.rand() for i in range(len(masterlist))]
                exlb = np.array([lambda x : x[i]/(rands[i] + x[i]) for i in range(len(masterlist))])
                exlbdt = np.array([lambda x,xd : xd[i]*rands[i]/((rands[i]+x[i])**2) for i in range(len(masterlist))])
                ...
                exlb = np.array([lambda x : x[i]/(1 + x[i]) for i in range(len(masterlist))])
                exlbdt = np.array([lambda x,xd : xd[i]*1/((1+x[i])**2) for i in range(len(masterlist))])
            else:
            ...
            return None
                                ...
                                #SurfMod(gamStar,gamDag,objective,interior_lbs,interior_ubs,exterior_lbfuns,exterior_ubfuns,exterior_lbfuns_derivative = [],exterior_ubfuns_derivatives = [],exchanged_metabolites,Name = None,deathrate = 0)
        surfmods[modelkey] = SurfMod(metaabs[model.name],GammaStarar,GammaDaggerar,lilgamma,internal_lower_bounds,internal_upper_bounds,exlb,exub,exterior_lbfuns_derivative = exlbdt,exterior_ubfuns_derivatives = exubdt,Name = model.name)
